...
| `metadata`                 | No       | Additional metadata like citations or trajectory information |
...
Used to reference information sources and provide attribution for content. Citations can be rendered as inline icons or text range annotations.
...
Used to capture an agent's reasoning steps or tool execution information, providing transparency into the agent's decision-making process.
