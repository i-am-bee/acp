...
3. **Await-based Pause**: Using the Await mechanism, agents can temporarily pause execution in the `awaiting` state to request additional information or actions from the client.
...
2. **Cancelled Execution**: The Await state can also be interrupted by cancellation. If a cancel request is received while an agent is in the `awaiting` state, it will transition to `cancelling` and then eventually move to the `cancelled` terminal state. This allows graceful termination of runs even when they are paused waiting for input.
