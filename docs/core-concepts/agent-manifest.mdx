...
The `input_content_types` and `output_content_types` fields specify the MIME types that an agent can handle.
...
If `input_content_types` and `output_content_types` are not specified in the decorator, they default to `["text/plain"]`:

```python
@server.agent()  # Uses default ["text/plain"] for both input and output
